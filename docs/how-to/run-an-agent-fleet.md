...
| `unknown provider_type — skipping` | A `type:` value the dispatcher doesn't know about — likely a typo. Supported types include `openai`, `openai-codex`, `ollama`, `simulated`, `exec`, `mcp`, `claude`. |
