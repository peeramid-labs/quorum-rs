...
| `NSED_TELEMETRY` | `telemetry.>` | Drain buffer for #309 telemetry. Stream name matches the `nsed-telemetry-forwarder::config::DEFAULT_STREAM_NAME` constant. Configured separately on dedicated telemetry node(s) via NATS placement tags — MUST NOT share storage with `nsed_results_*` on orchestrator hosts (load isolation + tenancy). Defaults: file storage, 24h time cap / 2 GB size cap, `Discard: old`, drained by the `nsed-telemetry-forwarder` durable consumer. Stream + placement detail ships in `ops/nats/telemetry-placement.md` (PR5 of the #309 stack). |
