...
server concepts; this one explains how the **client** should present them so a
user gets a Claude-Code-style chat instead of a room-picker.

The one-line version:

> **Policy is the model. The session is the thread.** A user picks a policy the
> way they pick a model, types a question, and gets an answer; continuing the
> conversation, resuming it later, and swapping the policy mid-chat are all
> properties of a client-owned *session*, not of a room they must choose first.
...
| **Where it lives / resume / who sees** | `room` | `room_id` = `session_id` (`x-nsed-session-id`) | *the session/thread* |
...
Two continuation models exist. They are not equivalent for a chat client:
...
For a chat that must **store, restore, and swap policy mid-conversation**, the
...
So the interactive client speaks **Chat Completions and keeps its own session
store**; the Responses API remains the cleaner fit for external stateful clients
that want server-held threads.

## What a session is, client-side

A session is a small client-owned record (not the existing `sessions.json`,
which is unrelated Claude-CLI-UUID plumbing):

```text
ChatSession {
  id, title, created, updated,
  active_policy,          // the "model" in force; recorded per turn too
  orchestrator,
  server_thread,          // x-nsed-session-id, for cheap same-policy continuation
  turns: [ { role, content, policy_id, job_id, ts } ],
}
```
...
chat session model does not preclude tool use; a client's tools ride down as
`user_tools`.
...
the deliberation UX — rounds, competing proposals, convergence, and the coming
*channel* model — will diverge from any generic coding-agent client over time.
That divergence is why the interactive client is built in-house rather than
borrowed. The design principle it commits to is the one at the top: **policy is
the model, the session is the thread.**
...
- [Glossary](../reference/glossary.md) — room, policy, session, job, effort.
