...
pub(super) async fn wizard_providers(
    ...
    loop {
        ...
        // API key — supports ${ENV_VAR} references resolved from .env
        let (api_key, resolved_key) = if needs_key {
            // Check preserved env for an existing key
    ...
    for p in providers {
        if p.provider_type == "exec" {
            ...
            for mi in &p.models {
                ...
                // Put Claude CLI at the top (tested_options)
                if p.id == "claude_cli" {
                    tested_options.push((
                        label,
                        p.id.clone(),
...
/// Build persona display options from agent presets for the multi-select UI.
...
pub(super) fn wizard_agents(
    ...
    for (i, name) in selected_names.iter().enumerate() {
        ...
        // If model_name is empty, ask user to type it (re-prompt until non-empty)
        ...
        // For exec providers, determine the command to run.
    ...
    // Diversity check — distinct models, not providers
