...
fn generate_output_content(
    ...
    if persona == "owner" {
        ...
        // Worker: account_seed holds the orchestrator URL
...
/// Print next-steps guidance after a successful init.
fn print_next_steps(
    persona: &str,
    compose_name: &str,
    app_port: u16,
...
pub async fn run_owner(
    ...
    let (app_port, nats_port, api_secret, worker_secret, account_seed) = if persona == "owner" {
        ...
        // NKey account seed — interactive: paste existing or generate new
        ...
        // Worker: orchestrator URL and bearer token
        ...
        // For worker, we store orchestrator_url as the seed and token as the secret.
        // Worker has no separate admin token — both slots use the same worker token.
